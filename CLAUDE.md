...
code/
  ...
  add_affiliations_to_registrants.R  # One-time: enriches data/registrants.csv with institution column
  generate_badges.R           # Generates badges.pdf (6 per page, front + QR back, duplex-ready)
...
data/
  ...
  registrants.csv     # Conference registrants (name, email, institution) — hand-edit institution as needed
  guests.csv          # Special guests/speakers (name, email, title, affiliation)

index.html            # Web app (served by GitHub Pages)
images/logo.png       # Conference logo (used in schedule.qmd and on name badges)
images/qr-schedule.png  # QR code linking to the web schedule (printed on badge backs)
...
Badges are generated by `code/generate_badges.R` and output to `badges.pdf`.

**Layout:** 6 per page (2 cols × 3 rows), each 4.25" × 3.67" on US Letter — fills the page edge-to-edge. Print on cardstock and cut; slide into standard lanyard badge holders.

**PDF structure:** front and back pages alternate (page 1 = fronts, page 2 = QR backs, page 3 = fronts, …). Print the entire PDF duplex (long-edge binding) so the QR code lands on the back of each badge. A final blank sheet (6 empty badges + QR back) is appended for walk-in registrants.

**Badge front:** dark blue banner at bottom with ISA logo, conference name, and "June 3–5 · Washington, DC". Name (large, bold) and institution in the white area above. Long names auto-scale down to fit.

**Badge back:** centered QR code linking to the web schedule with "Scan for full schedule" caption.

**Data sources:**
- `data/registrants.csv` — `name`, `email`, `institution` columns. The `institution` column is populated once by running `Rscript code/add_affiliations_to_registrants.R`, then hand-edited for any blanks.
- `data/guests.csv` — `name`, `email`, `title`, `affiliation` columns (affiliation used directly).

**To regenerate badges** (after editing `registrants.csv` or `guests.csv`):
```
Rscript code/generate_badges.R
```

**To re-populate affiliations from scratch** (only needed if `registrants.csv` loses its `institution` column):
```
Rscript code/add_affiliations_to_registrants.R
```
Then hand-edit any blanks reported in the console before running `generate_badges.R`.
