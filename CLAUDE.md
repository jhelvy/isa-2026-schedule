...
code/
  ...
  3generate_json.R            # Generates schedule_data.json, excursions_data.json, bios_data.json, awards_data.json
...
excursions_data.json  # OUTPUT: excursion details for web app (generated by 3generate_json.R)
...
- The `time_order` for each event is computed from seconds-since-midnight via `case_when` — if a new event time is added to `overview.csv`, add a matching entry to the `case_when` block in `3generate_json.R`. Current Wednesday entries: 8:00 AM → 0.10, 9:30 AM → 0.12, 12:00 PM → 0.30.
